...
const Sidebar: React.FC<SidebarProps> = ({ idioma, onInicioClick }) => {
    ...
    return (
        <>
            {/* Botón menú en móvil */}
            <button
                className="fixed top-4 left-4 z-50 p-2 md:hidden bg-teal-500 rounded-full shadow-lg"
                onClick={() => setOpen(true)}
                aria-label="Abrir menú"
            >
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" d="M9 6l6 6-6 6" />
                </svg>
            </button>

            {/* Overlay en móvil */}
            <div
                className={`fixed inset-0 bg-black bg-opacity-60 z-40 transition-opacity duration-300 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"} md:hidden`}
                onClick={() => setOpen(false)}
            />

            {/* Sidebar */}
            <aside className={`
                fixed top-0 left-0 h-screen min-h-screen w-64
                bg-white dark:bg-black border-r border-gray-300 dark:border-white z-50
